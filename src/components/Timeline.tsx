...
const Timeline = ({ events }: Props) => {
    ...
    return (
        <ol className="border-l border-surface-100">
            {events.reverse().map((event, index) => (
                <li key={index} className="flex flex-col mb-10 ml-4">
                    <div className="flex flex-row mt-1.5">
                        <div className="w-4 h-4 rounded-full -ml-6 border border-black border-2 bg-surface-100"></div>
                    ...
                    {event.link && (
                        ...
                        focus:text-blue-700 bg-surface-100 text-gray-400 
                        border-2
                        border-gray-600 hover:text-white hover:bg-surface-000 focus:ring-background">{event.link.text}</GatsbyLink>
                        )}
                    </div>
                </li>
            ))}
