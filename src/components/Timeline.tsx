...
const Timeline = ({ events }: Props) => {
    ...
    return (
        <ol className="border-l border-surface">
            {events.reverse().map((event, index) => (
                <li key={index} className="flex flex-col mb-10 ml-4">
                    <div className="flex flex-row mt-1.5">
                        <div className="w-4 h-4 rounded-full -ml-6 border border-black border-2 bg-surface"></div>
                    ...
                    {event.link && (
                        ...
                        focus:text-blue-700 bg-surface text-gray-400 
                        border-2
                        border-gray-600 hover:text-white hover:bg-background focus:ring-background">{event.link.text}</GatsbyLink>
                        )}
                    </div>
                </li>
            ))}
